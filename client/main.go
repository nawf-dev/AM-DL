...
func getUrlSong(songUrl string, token string) (string, error) {
	storefront, songId := support.ParseSongURL(songUrl)
...
func getUrlArtistName(artistUrl string, token string) (string, string, error) {
	storefront, artistId := support.ParseArtistURL(artistUrl)
...
func writeCover(sanAlbumFolder, name string, url string) (string, error) {
	...
	exists, err := support.FileExists(covPath)
	if err != nil {
...
func convertIfNeeded(track *task.Track) {
	...
	ffmpegPath, err := support.ResolveExecutable(Config.FFmpegPath)
	if err != nil {
		fmt.Printf("ffmpeg not found at '%s'; skipping conversion.\n", Config.FFmpegPath)
		return
	}

	args, err := buildFFmpegArgs(ffmpegPath, srcPath, outPath, targetFmt, Config.ConvertExtraArgs)
	if err != nil {
...
func ripTrack(track *task.Track, token string, mediaUserToken string) {
	...
	if track.Type == "music-videos" {
		...
		if _, err := support.ResolveExecutable("mp4decrypt"); err != nil {
			fmt.Println("mp4decrypt is not found in PATH/tools, skip MV dl")
			counter.Success++
			return
		}
	...
	songName := strings.NewReplacer(
		...
		"{SongName}", support.LimitString(track.Resp.Attributes.Name, Config.LimitMax),
	...
	existsOriginal, err := support.FileExists(trackPath)
	if err != nil {
	...
	if considerConverted {
		existsConverted, err2 := support.FileExists(convertedPath)
	...
	mp4BoxPath, err := support.ResolveExecutable("MP4Box")
	if err != nil {
		fmt.Printf("Embed skipped: %v\n", err)
		counter.Error++
		return
	}
...
func ripStation(albumId string, token string, storefront string, mediaUserToken string) error {
	...
	playlistFolder := strings.NewReplacer(
		"{ArtistName}", "Apple Music Station",
		"{PlaylistName}", support.LimitString(station.Name, Config.LimitMax),
	...
	if Config.SaveAnimatedArtwork && meta.Data[0].Attributes.EditorialVideo.MotionSquare.Video != "" {
		...
		if err != nil {
			...
			exists, err := support.FileExists(filepath.Join(playlistFolderPath, "square_animated_artwork.mp4"))
			if err != nil {
	...
	if station.Type == "stream" {
		...
		songName := strings.NewReplacer(
			"{SongId}", station.ID,
			"{SongNumer}", "01",
			"{SongName}", support.LimitString(station.Name, Config.LimitMax),
			"{DiscNumber}", "1",
			"{TrackNumber}", "1",
			"{Quality}", "256Kbps",
			"{Tag}", "",
		...
		exists, _ := support.FileExists(trackPath)
		if exists {
			counter.Success++
...
func ripAlbum(albumId string, token string, storefront string, mediaUserToken string, urlArg_i string) error {
	...
	if Config.ArtistFolderFormat != "" {
		if len(meta.Data[0].Relationships.Artists.Data) > 0 {
			singerFoldername = strings.NewReplacer(
				"{UrlArtistName}", support.LimitString(meta.Data[0].Attributes.ArtistName, Config.LimitMax),
				"{ArtistName}", support.LimitString(meta.Data[0].Attributes.ArtistName, Config.LimitMax),
	...
	albumFolderName = strings.NewReplacer(
		...
		"{ArtistName}", support.LimitString(meta.Data[0].Attributes.ArtistName, Config.LimitMax),
		"{AlbumName}", support.LimitString(meta.Data[0].Attributes.Name, Config.LimitMax),
	...
	if Config.SaveAnimatedArtwork && meta.Data[0].Attributes.EditorialVideo.MotionDetailSquare.Video != "" {
		...
		if err != nil {
			...
			exists, err := support.FileExists(filepath.Join(albumFolderPath, "square_animated_artwork.mp4"))
			if err != nil {
				fmt.Println("Failed to check if animated artwork square exists.")
			}
			if exists {
		...
		if err != nil {
			...
			exists, err := support.FileExists(filepath.Join(albumFolderPath, "tall_animated_artwork.mp4"))
			if err != nil {
...
func ripPlaylist(playlistId string, token string, storefront string, mediaUserToken string) error {
	...
	playlistFolder := strings.NewReplacer(
		"{ArtistName}", "Apple Music",
		"{PlaylistName}", support.LimitString(meta.Data[0].Attributes.Name, Config.LimitMax),
	...
	if Config.SaveAnimatedArtwork && meta.Data[0].Attributes.EditorialVideo.MotionDetailSquare.Video != "" {
		...
		if err != nil {
			...
			exists, err := support.FileExists(filepath.Join(playlistFolderPath, "tall_animated_artwork.mp4"))
			if err != nil {
				fmt.Println("Failed to check if animated artwork tall exists.")
			}
			if exists {
	...
	for {
		for albumNum, urlRaw := range os.Args {
			...
			if strings.Contains(urlRaw, "/music-video/") {
				...
				storefront, albumId = support.ParseMusicVideoURL(urlRaw)
			...
			if strings.Contains(urlRaw, "/song/") {
				fmt.Printf("Song->")
				storefront, songId := support.ParseSongURL(urlRaw)
			...
			if strings.Contains(urlRaw, "/album/") {
				fmt.Println("Album")
				storefront, albumId = support.ParseAlbumURL(urlRaw)
				...
				storefront, albumId = support.ParsePlaylistURL(urlRaw)
				...
				fmt.Printf("Station")
				storefront, albumId = support.ParseStationURL(urlRaw)
				if len(Config.MediaUserToken) <= 50 {
...
func mvDownloader(adamID string, saveDir string, token string, storefront string, mediaUserToken string, track *task.Track) error {
	...
	exists, _ := support.FileExists(mvOutPath)
	if exists {
